-- Add phone-only web sessions while retaining Zalo Mini App sessions and ZBS delivery.
alter table public.participant_sessions drop constraint if exists participant_sessions_auth_method_check;
alter table public.participant_sessions add constraint participant_sessions_auth_method_check check (auth_method in ('preview', 'zalo', 'phone'));

alter table public.campaign_participants drop constraint if exists campaign_participants_registration_source_check;
alter table public.campaign_participants add constraint campaign_participants_registration_source_check check (registration_source in ('import', 'zalo_guest', 'unlisted', 'admin', 'phone_guest'));

update public.program_settings set value = value - array['participantAuthMode', 'zaloOaId', 'oaRequired'] where key = 'system_env_config' and jsonb_typeof(value) = 'object';

drop function if exists public.spin_once(text, text, boolean, text);
          ...
          r.scope = 'guest' and (
            ...
            or exists (
              ...
              where cp.campaign_id = v_campaign_id
                ...
                and cp.registration_source in ('zalo_guest', 'unlisted', 'phone_guest')
            )
          )
        )
        or exists (
...
revoke all on function public.spin_once(text, text, text) from public, anon, authenticated;
grant execute on function public.spin_once(text, text, text) to service_role;
